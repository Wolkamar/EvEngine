...
//{
//public:
//	GameObject(std::string name, float x, float y, float sx, float sy, sf::Color color, float radius)
//		: G(name, x, y, sx, sy, color, radius)
//	{
//	}
//
//	GameObject(std::string name, float x, float y, float sx, float sy, sf::Color color, float width, float height)
//		: G(name, x, y, sx, sy, color, width, height)
//	{
//	}
//};
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			std::string cricleName = circles[i]->getName();
			//std::string cricleName = "asd";

			float charsNumber = cricleName.size();


			sf::Text text = sf::Text();
			text.setString(cricleName);
			text.setFont(font);
